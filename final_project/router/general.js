...
// Promise callback to the console
...
getBooks.then(
    ...
    (err) => console.log("Error getting books")
    );
...
let getBookByISBN = new Promise((resolve, reject) => {
    setTimeout(() => {
        try {
            let isbn = parseInt(prompt("What is the book number?"));
            resolve(books[isbn]);
        } catch (err) {
            reject(err);
        }
...
getBookByISBN.then(
    (isbn) => console.log(books[isbn]),
    (err) => console.log("Error getting book")
    );
